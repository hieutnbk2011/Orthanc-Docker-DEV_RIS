...
$load = 0;
$selectlist = "";
$server = new OrthancAPI();
$orthanc_hosts = $server->getAllHosts();  // inits the server and sets the selected server to the current or default host. returns an array of all hosts.
//$telerad_contract = $data['telerad_contract'];

$selectlistoptions = [];

$selectlist = '<select id="orthanc_host" name="orthanc_host">';
...
<div id="studylist">
          ...
          <div class="col">
          <form id="orthancPACS" name="orthancPACS" action="/PACS/changeserver" method="POST">
          	<?php
          	    echo $selectlist;
          	 ?>
          	 @csrf
...
<?php
$user = Auth::user();
$users = DB::connection('mysql2')->select('SELECT * from patients WHERE mrn = ?', [$user->patientid] );
print_r($users);
 ?>
</div>
...
$.ajaxSetup({
    ...
    'Authorization': 'Bearer a47VSfjqKSjNqXb1A7chpyCLC8PyZByGNqnsAeuy'
	},
...
function showMessage(title, message) {
	// alert(title);
	// alert(message);
	// // accumulate messages if called twice.
	// if ($(".fadeInDown").length !=0) {
	// title = $(".fadeInDown h3").html() + "<br>" + title;
	// message = "- " + $(".fadeInDown div").html( )+ "<br>- " + message;
	// }
	showmodal(title, message);
	// $("#logregforms").html('<div class="container"><div class="wrapper fadeInDown"><i class="fas fa-window-close logregformsclose"></i><h3>' +  title + '</h3><div>' + message + '</div></div></div>')
