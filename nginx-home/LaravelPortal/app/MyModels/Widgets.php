    ...
    public static function studiesSearchForm () {
		...
		<form name="searchform" id="searchform" class="divtable row">
            ...
            <div class="col-sm-1 nopadding"><span class="narrowmedia">Description:  </span><input name="data-description" data-orthanc = "StudyDescription" class="searchparam" type="text"></div>
             <div class="col-sm-1 nopadding"><span class="narrowmedia">Referring:  </span><input name="data-referring_physician" id="data-referring_physician" data-orthanc = "ReferringPhysicianName" class="searchparam" type="text"></div>
            <div class="col-sm-1 nopadding"><span class="narrowmedia">Modality:  </span><select name="data-modality" data-orthanc = "Modality" class="searchparam"><option value="">All</option><option value="MR">MR</option>><option value="PT">PT</option><option value="CT">CT</option><option value="US">US</option><option value="NM">NM</option><option value="CR">CR</option><option value="OT">OT</option></select></div>
		...
		<form name="searchform" class="searchform divtable row">
		    <div class="col-sm-2"></div>
            <div class="col-sm-2 nopadding"><span class="narrowmedia">InstitutionName:  </span><select name="data-institution_name" id="institution_name-status" class="searchparam" data-orthanc = "InstitutionName" form="searchform"><option value = "">All</option><option value = "Cayman Medical Ltd.">Cayman Medical Ltd.</option><option value = "Test">TEST</option></select></div>
...
<button type="button" data-dbsearch class="uibuttonsmallred">Search Database</button></div></div>';
    ...
    public static function dateRadioSelectorStudies($formname, $worklistcontainer, $dbsearch) {
    	...
    	return '
            ...
            <div data-searchdb = "' . $dbsearch . '" class = "col-sm-12" style="text-align:center;">ALL: <input class = "changestudydate" name = "changestudydate" type = "radio" value = "0">10 years: <input class = "changestudydate" name = "changestudydate" type = "radio" value = "3650">  1 year: <input class = "changestudydate" name = "changestudydate" type = "radio" value = "365">  30 days:  <input class = "changestudydate" name = "changestudydate" type = "radio" value = "30">  7 days:  <input class = "changestudydate" name = "changestudydate" type = "radio" value = "7">  1 day:  <input class = "changestudydate" name = "changestudydate" type = "radio" value = "1">&nbsp;&nbsp;<input type="button" class ="clearsearchform uibuttonsmallred" value="Clear" title="Clear" data-target = "'  . $worklistcontainer .  '">
    ...
    public static function checkAuth($path) {
		...
		if ((count(array_intersect(Session::get("user_roles"), [1])) > 0)) {
			...
			self::$omitdelete = true;
			DatabaseFactory::logVariable('root:  ' . $validroot . '  path:'  . $path . '  validpath:' .$text  );
		}
    ...
    public static function listFilesInPath($path, $callback) {
    	...
    	$errors = "";
    	DatabaseFactory::logVariable($path);
...
DatabaseFactory::logVariable($status);
DatabaseFactory::logVariable($orthanc);
...
return $html;
    ...
    // DatabaseFactory::logVariable("Limit:  " . $limit . ", Page:  " . $current_page . ", Count:  " . $count );
